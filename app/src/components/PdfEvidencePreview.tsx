      ...
      {!loading && !failed && !tooManyPages && pages.length > 0 && <>
        ...
        {pages.map(({ number, page, scale }) => {
          ...
          return <section key={number} aria-label={`${copy.page} ${number}`} style={{ marginBlock: '1.5rem' }}>
            ...
            <p role="status" aria-live="polite" aria-atomic="true">{copy.redactionStatus(redactions[number]?.length ?? 0, number)}</p>
          </section>
        })}
...
const pdfText = {
  en: { preview: 'Preview and redact PDF', close: 'Close PDF preview', explanation: 'PDF pages are rendered on this device. Drag over sensitive information on each page, or use the keyboard-accessible central redaction button for a fixed center area. The original stays unchanged and is never uploaded.', loading: 'Loading PDF pages…', rendering: 'Rendering PDF pages…', loadError: 'The PDF could not be rendered. Download the original to inspect it.', pageLimit: `This preview supports up to ${MAX_PAGES} pages. Download the original and redact it with a trusted PDF tool.`, warningTitle: 'Review every page before sharing.', warning: 'The exported copy is image-only: text and vector detail are flattened and may be less sharp. Check every page at full size to confirm all sensitive information is covered. Redaction does not remove information that remains visible outside the black areas.', page: 'Page', centralRedaction: 'Add central redaction', clear: 'Clear redactions', redactionStatus: (count: number, page: number) => count ? `${count} redaction${count === 1 ? '' : 's'} applied to page ${page}.` : `No redactions on page ${page}.`, export: 'Download flattened redacted PDF', exporting: 'Preparing PDF…', exportError: 'The redacted PDF could not be created. Try again.' },
  ms: { preview: 'Pratonton dan redaksi PDF', close: 'Tutup pratonton PDF', explanation: 'Halaman PDF dipaparkan pada peranti ini. Seret pada maklumat sensitif di setiap halaman, atau gunakan butang redaksi tengah yang boleh diakses dengan papan kekunci. Fail asal tidak berubah dan tidak dimuat naik.', loading: 'Memuatkan halaman PDF…', rendering: 'Memaparkan halaman PDF…', loadError: 'PDF tidak dapat dipaparkan. Muat turun fail asal untuk menyemaknya.', pageLimit: `Pratonton ini menyokong sehingga ${MAX_PAGES} halaman. Muat turun fail asal dan redaksikannya dengan alat PDF yang dipercayai.`, warningTitle: 'Semak setiap halaman sebelum berkongsi.', warning: 'Salinan eksport hanya mengandungi imej: teks dan perincian vektor diratakan dan mungkin kurang jelas. Periksa setiap halaman pada saiz penuh untuk memastikan semua maklumat sensitif ditutup. Redaksi tidak membuang maklumat yang masih kelihatan di luar kawasan hitam.', page: 'Halaman', centralRedaction: 'Tambah redaksi tengah', clear: 'Kosongkan redaksi', redactionStatus: (count: number, page: number) => count ? `${count} redaksi pada halaman ${page}.` : `Tiada redaksi pada halaman ${page}.`, export: 'Muat turun PDF redaksi yang diratakan', exporting: 'Menyediakan PDF…', exportError: 'PDF redaksi tidak dapat dibuat. Cuba lagi.' },
} as const
