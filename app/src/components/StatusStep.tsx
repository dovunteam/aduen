...
export function StatusStep({ onBack, onStatusChange }: { onBack: () => void; onStatusChange: (status: CaseRecordStatus) => void }) {
  ...
  return <section className="page form-page status-page">
    <div className="status-heading"><div><div className="eyebrow">Tuntiva Status</div><h1>Record what happens next.</h1><p className="lede">Tuntiva does not monitor the merchant or submit on your behalf. Add external events here so the case record remains complete.</p></div><div className={`status-badge ${record.status}`}><span>Current status</span><strong>{record.status.replaceAll('_', ' ')}</strong></div></div>
    <form onSubmit={submit}>
      ...
      <div className="form-section"><SectionTitle number="03" title="Outcome" copy="Close the case only when the external result is known." /><div className="fields"><label>Recorded outcome<select disabled={terminal} value={record.outcome} onChange={(event) => update('outcome', event.target.value as SubmissionRecord['outcome'])}><option value="">No final outcome yet</option><optgroup label="Resolved"><option value="refund">Refund received</option><option value="replacement">Replacement received</option><option value="repair">Repair completed</option><option value="delivery">Delivery completed</option></optgroup><optgroup label="Closed without requested remedy"><option value="rejected">Rejected</option><option value="redirected">Redirected elsewhere</option><option value="withdrawn">Withdrawn by me</option><option value="unresolved">Unresolved</option></optgroup></select></label></div></div>
