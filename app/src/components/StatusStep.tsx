...
export function StatusStep({ locale, onBack, onStatusChange }: { locale: Locale; onBack: () => void; onStatusChange: (status: CaseRecordStatus) => void }) {
  ...
  function submit(event: FormEvent) {
    ...
    try {
      const saved = saveSubmission({ ...record, status })
      onStatusChange(status === 'ready' ? 'approved' : status)
      setRecord(saved); setMessage(text.saved)
    } catch { setError(locale === 'ms' ? 'Status tidak dapat disimpan. Semak storan pelayar dan cuba lagi.' : 'Status could not be saved. Check browser storage and retry.') }
  }
