...
type Props = { locale: Locale; draft: CaseDraft; onBack: () => void; onDelete: () => Promise<void>; hostedConfigured: boolean; signedIn: boolean; identityError: boolean; onSignIn: () => void | Promise<void>; onSaveHosted: () => Promise<void>; onListHosted: (cursor?: string) => Promise<CasePage>; onDeleteHosted: (id: string, revision: number) => Promise<void>; onDeleteHostedAccountData: () => Promise<void>; onExportHostedAccountData: () => Promise<void>; onImportHosted: (item: StoredCase) => Promise<void> }

export function DataControls({ locale, draft, onBack, onDelete, hostedConfigured, signedIn, identityError, onSignIn, onSaveHosted, onListHosted, onDeleteHosted, onDeleteHostedAccountData, onExportHostedAccountData, onImportHosted }: Props) {
  ...
  return <section className="page narrow-page data-page">
    ...
    {hostedConfigured && signedIn && <HostedAccountControls locale={locale} busy={hostedSaving || exporting || deleting} canImport={hostedImportAvailable} onList={onListHosted} onDelete={onDeleteHosted} onDeleteAll={onDeleteHostedAccountData} onExport={onExportHostedAccountData} onImport={onImportHosted} />}
...
function HostedAccountControls({ locale, busy, canImport, onList, onDelete, onDeleteAll, onExport, onImport }: { locale: Locale; busy: boolean; canImport: boolean; onList: (cursor?: string) => Promise<CasePage>; onDelete: (id: string, revision: number) => Promise<void>; onDeleteAll: () => Promise<void>; onExport: () => Promise<void>; onImport: (item: StoredCase) => Promise<void> }) {
  const text = dataText[locale]
  ...
  const accountDeletionText = locale === 'ms'
    ? { button: 'Padam semua data dihoskan', confirm: 'Padam secara kekal semua rekod kes dan sejarah mutasi dihoskan dalam akaun Aduen ini? Ini tidak memadam data dalam pelayar ini, akaun log masuk luaran anda atau salinan dalam sandaran perkhidmatan.', busy: 'Memadam data dihoskan?', done: 'Semua data kes dan sejarah mutasi dihoskan telah dipadam. Data dalam pelayar, akaun log masuk luaran dan salinan dalam sandaran perkhidmatan tidak terjejas.', failed: 'Data dihoskan tidak dapat dipadam. Cuba lagi.' }
    : { button: 'Delete all hosted data', confirm: 'Permanently delete every hosted case record and mutation history for this Aduen account? This does not delete data in this browser, your external sign-in account, or copies in service backups.', busy: 'Deleting hosted data?', done: 'All hosted case data and mutation history were deleted. Browser data, your external sign-in account, and copies in service backups are unaffected.', failed: 'Hosted data could not be deleted. Please retry.' }
  const accountExportText = locale === 'ms'
    ? { button: 'Eksport semua data dihoskan', copy: 'Fail ini mengandungi semua rekod kes berstruktur dalam akaun anda. Bukti dan fail setempat tidak disertakan.', busy: 'Menyediakan eksport…', done: 'Muat turun data akaun dihoskan telah disediakan.', failed: 'Data dihoskan tidak dapat dieksport. Cuba lagi.' }
    : { button: 'Export all hosted data', copy: 'This file contains every structured case record in your account. Browser evidence and files are not included.', busy: 'Preparing export…', done: 'The hosted account data download is ready.', failed: 'Hosted data could not be exported. Please retry.' }
  ...
  async function exportAllCopies() {
    setExportingAll(true); setError(''); setExportedAll(false)
    try { await onExport(); setExportedAll(true) }
    catch { setError(accountExportText.failed) }
  ...
  return <section className="hosted-account" aria-labelledby="hosted-account-title">
    ...
    {exportedAll && <p role="status">{accountExportText.done}</p>}
...
function auditLabel(event: LocalAuditEvent, locale: Locale): string {
  ...
  if (event.action === 'hosted_account_data_exported') return locale === 'ms' ? 'Eksport semua data dihoskan' : 'All hosted account data exported'
