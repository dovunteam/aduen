...
export function DataControls({ locale, draft, onBack, onDelete }: Props) {
  ...
  return <section className="page narrow-page data-page">
    ...
    <section className="data-action"><div><h2>{text.exportTitle}</h2><p>{text.exportCopy}</p></div><button className="primary" disabled={(!hasData && !inventoryFailed) || exporting || deleting} onClick={exportData}>{exporting ? text.preparing : text.export} <span aria-hidden="true">↓</span></button></section>
...
const dataText = {
  en: { eyebrow: 'Privacy controls', title: <>Your data stays<br />under your control.</>, lede: 'Your case record, evidence, and working files stay in this browser on this device. Aduen does not use an account or send case data to a server. Clearing browser data or changing devices can remove access, so export a copy if you need one.', caseDraft: 'Case draft', saved: '1 saved', none: 'None', evidence: 'Evidence originals', candidates: 'Derived fact candidates', packs: 'Complaint pack versions', submission: 'Submission status', consent: 'Privacy notice consent', accepted: 'Accepted', unavailable: 'Unavailable', storage: 'Storage', browser: 'This device only', retentionTitle: 'Automatic deletion', retentionChoice: 'Delete local data after', retentionOff: 'No automatic deletion', retention30: '30 days', retention90: '90 days', retention365: '12 months', retentionNone: 'No automatic deletion is scheduled.', retentionSet: (date: string) => 'Local data will be deleted when Aduen is next opened on or after ' + date + '.', auditTitle: 'Local activity history', auditCopy: 'Review recent sensitive actions recorded on this device. This history stays in this browser.', auditEmpty: 'No sensitive actions recorded yet.', auditView: (count: number) => `View ${count} recorded ${count === 1 ? 'action' : 'actions'}`, exportTitle: 'Export the complete case', exportCopy: 'Download a ZIP containing the structured case record, evidence metadata and hashes, submission status, and every original evidence file.', preparing: 'Preparing archive…', export: 'Export ZIP', deleteTitle: 'Delete local case data', deleteCopy: 'Permanently remove the case record, evidence metadata, evidence originals, packs, and status from this browser on this device.', delete: 'Delete all case data', back: '← Return to case', deleteConfirm: 'Delete the case draft, every evidence original, and its status record from this browser? This cannot be undone.' },
  ms: { eyebrow: 'Kawalan privasi', title: <>Data anda kekal<br />di bawah kawalan anda.</>, lede: 'Rekod kes, bukti dan fail kerja anda kekal dalam pelayar pada peranti ini. Aduen tidak menggunakan akaun atau menghantar data kes ke pelayan. Memadam data pelayar atau menukar peranti boleh menyebabkan anda kehilangan akses. Eksport salinan jika anda memerlukannya.', caseDraft: 'Draf kes', saved: '1 disimpan', none: 'Tiada', evidence: 'Fail asal bukti', candidates: 'Calon fakta terbitan', packs: 'Versi pek aduan', submission: 'Status penyerahan', consent: 'Persetujuan notis privasi', accepted: 'Diterima', unavailable: 'Tidak tersedia', storage: 'Storan', browser: 'Peranti ini sahaja', retentionTitle: 'Pemadaman automatik', retentionChoice: 'Padam data setempat selepas', retentionOff: 'Tiada pemadaman automatik', retention30: '30 hari', retention90: '90 hari', retention365: '12 bulan', retentionNone: 'Tiada pemadaman automatik dijadualkan.', retentionSet: (date: string) => 'Data setempat akan dipadam apabila Aduen dibuka pada atau selepas ' + date + '.', auditTitle: 'Sejarah aktiviti setempat', auditCopy: 'Semak tindakan sensitif terkini yang direkodkan pada peranti ini. Sejarah ini kekal dalam pelayar ini.', auditEmpty: 'Tiada tindakan sensitif direkodkan lagi.', auditView: (count: number) => `Lihat ${count} tindakan direkodkan`, exportTitle: 'Eksport kes lengkap', exportCopy: 'Muat turun ZIP yang mengandungi rekod kes berstruktur, metadata dan cincangan bukti, status penyerahan, serta setiap fail bukti asal.', preparing: 'Menyediakan arkib…', export: 'Eksport ZIP', deleteTitle: 'Padam data kes setempat', deleteCopy: 'Padam rekod kes, metadata bukti, fail asal bukti, pek dan status secara kekal daripada pelayar pada peranti ini.', delete: 'Padam semua data kes', back: '← Kembali ke kes', deleteConfirm: 'Padam draf kes, setiap fail asal bukti, dan rekod status daripada pelayar ini? Tindakan ini tidak boleh dibatalkan.' },
} as const
