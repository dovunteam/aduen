...
type Props = { locale: Locale; draft: CaseDraft; onBack: () => void; onDelete: () => Promise<void>; hostedConfigured: boolean; signedIn: boolean; identityError: boolean; onSignIn: () => void | Promise<void>; onSaveHosted: () => Promise<void>; onListHosted: (cursor?: string) => Promise<CasePage>; onDeleteHosted: (id: string, revision: number) => Promise<void> }

export function DataControls({ locale, draft, onBack, onDelete, hostedConfigured, signedIn, identityError, onSignIn, onSaveHosted, onListHosted, onDeleteHosted }: Props) {
  ...
  return <section className="page narrow-page data-page">
    ...
    {hostedConfigured && signedIn && <HostedAccountControls locale={locale} busy={hostedSaving || exporting || deleting} onList={onListHosted} onDelete={onDeleteHosted} />}
...
function HostedAccountControls({ locale, busy, onList, onDelete }: { locale: Locale; busy: boolean; onList: (cursor?: string) => Promise<CasePage>; onDelete: (id: string, revision: number) => Promise<void> }) {
  const text = dataText[locale]
