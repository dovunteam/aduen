...
export function PackStep({ initialPack, onBack, onContinue, onApproved }: Props) {
  ...
  return <section className="page form-page pack-page">
    <div className="eyebrow">Buktiva Pack · Version {pack.version}</div><h1>Review before export.</h1>
    <p className="lede">This pack is a factual case summary for your review. It is not sent anywhere by Buktiva, and exporting it does not submit a complaint.</p>
    <div className="pack-layout"><article className="pack-document">
      <header><span>BUKTIVA CASE PACK</span><small>Draft · {new Date(pack.createdAt).toLocaleString('en-MY')}</small></header>
    ...
    <div className="actions split"><button className="secondary" onClick={onBack}>← Buktiva Check</button><button className="primary" disabled={!pack.approvedAt} onClick={onContinue}>Track external status <span>→</span></button></div>
  </section>
}
