...
function handoffText(destination: ScopeHandoff['destination'], locale: Locale) {
  ...
  return locale === 'ms'
    ? { link: 'Buka proses aduan Bank Negara Malaysia', note: (checked: string) => `Adu kepada Unit Aduan penyedia perkhidmatan kewangan terlebih dahulu. BNM menyatakan aduan yang belum dirujuk kepada penyedia tidak diterima. Sumber disemak ${checked}.` }
    : { link: 'Open Bank Negara Malaysia complaint process', note: (checked: string) => `Complain to the financial service provider’s Complaints Unit first. BNM says it will not accept complaints not first referred to the provider. Source checked ${checked}.` }
}
