...
export function OutOfScopeStep({ locale, draft, assessment, onEdit, onDelete }: { locale: Locale; draft: CaseDraft; assessment: ScopeAssessment; onEdit: () => void; onDelete: () => Promise<void> }) {
  ...
  const handoffCopy = handoff ? handoffText(handoff.destination, locale) : null
...
function handoffText(destination: ScopeHandoff['destination'], locale: Locale) {
  if (destination === 'caam') return locale === 'ms'
    ? { link: 'Buka panduan aduan CAAM/FlySmart', note: (checked: string) => `Hubungi syarikat penerbangan, lapangan terbang atau penyedia dahulu. CAAM menyatakan aduan boleh dibuat melalui FlySmart jika tidak selesai selepas 30 hari. Sumber disemak ${checked}.` }
    : { link: 'Open CAAM/FlySmart complaint guidance', note: (checked: string) => `Contact the airline, airport, or provider first. CAAM says a FlySmart complaint may be made if it is unresolved after 30 days. Source checked ${checked}.` }
  return locale === 'ms'
    ? { link: 'Buka proses aduan Bank Negara Malaysia', note: (checked: string) => `Adu kepada Unit Aduan penyedia perkhidmatan kewangan terlebih dahulu. BNM menyatakan aduan yang belum dirujuk kepada penyedia tidak diterima. Jika tiada respons selepas 14 hari, anda boleh merujuk kes kepada BNMLINK; jika tidak berpuas hati dengan keputusan, rujuk saluran tebus rugi yang berkaitan. Sumber disemak ${checked}.` }
    : { link: 'Open Bank Negara Malaysia complaint process', note: (checked: string) => `Complain to the financial service provider’s Complaints Unit first. BNM says it will not accept complaints not first referred to the provider. If there is no response after 14 days, you may refer the case to BNMLINK; if you are dissatisfied with the decision, use the relevant redress channel. Source checked ${checked}.` }
}
