...
export function ExtractionStep({ locale, initialExtractions, evidence, onBack, onContinue }: Props) {
  ...
  return <section className="page form-page extraction-page">
    ...
    <div className="extraction-summary"><div><strong>{candidates.length}</strong><span>{text.candidates}</span></div><div><strong>{pending}</strong><span>{text.awaiting}</span></div><div><strong>{extractorVersions.join(', ') || 'plain-text-v7'}</strong><span>{text.version}</span></div></div>
