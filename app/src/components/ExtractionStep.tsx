...
export function ExtractionStep({ locale, initialExtractions, evidence, onBack, onContinue }: Props) {
  ...
  return <section className="page form-page extraction-page">
    ...
    <div className="extraction-summary"><div><strong>{candidates.length}</strong><span>{text.candidates}</span></div><div><strong>{pending}</strong><span>{text.awaiting}</span></div><div><strong>plain-text-v4</strong><span>{text.version}</span></div></div>
...
const extractionText = {
  en: { eyebrow: 'Extracted-fact confirmation', title: 'Check every candidate.', lede: 'Aduen found possible facts in plain-text evidence. They are derived suggestions—not case facts—until you confirm or correct them.', candidates: 'candidates found', awaiting: 'awaiting your decision', version: 'current extractor', evidenceFile: 'Evidence file', confidence: 'pattern confidence · unverified', candidateValue: 'Candidate value', confirmed: (value: string) => `Confirmed as ${value}`, rejected: 'Rejected — not used as a fact', confirm: 'Confirm', correction: ' correction', reject: 'Reject', privacyLead: 'Originals are unchanged.', privacy: 'Your decision is stored on the derived candidate with both the extracted value and any corrected value retained.', back: '← Evidence', continue: 'Continue to Aduen Check' },
  ms: { eyebrow: 'Pengesahan fakta diekstrak', title: 'Semak setiap calon.', lede: 'Aduen menemui kemungkinan fakta dalam bukti teks biasa. Ia ialah cadangan terbitan—bukan fakta kes—sehingga anda mengesahkan atau membetulkannya.', candidates: 'calon ditemui', awaiting: 'menunggu keputusan anda', version: 'pengekstrak semasa', evidenceFile: 'Fail bukti', confidence: 'keyakinan corak · belum disahkan', candidateValue: 'Nilai calon', confirmed: (value: string) => `Disahkan sebagai ${value}`, rejected: 'Ditolak — tidak digunakan sebagai fakta', confirm: 'Sahkan', correction: ' pembetulan', reject: 'Tolak', privacyLead: 'Fail asal tidak berubah.', privacy: 'Keputusan anda disimpan pada calon terbitan dengan nilai yang diekstrak dan sebarang nilai yang dibetulkan dikekalkan.', back: '← Bukti', continue: 'Teruskan ke Semakan Aduen' },
} as const
