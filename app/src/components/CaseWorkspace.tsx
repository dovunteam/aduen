  ...
  return <section className="page workspace-page">
    ...
    <div className="workspace-layout">
      <div className="workspace-main">
        ...
        <ol className="workspace-journey" aria-label={ms ? 'Peringkat kes' : 'Case stages'}>{labels.map((label, index) => <li key={label} className={index < stage ? 'completed' : undefined} aria-current={index === stage ? 'step' : undefined}><span aria-hidden="true">{String(index + 1).padStart(2, '0')}</span>{label}</li>)}</ol>
