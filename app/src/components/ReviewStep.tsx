...
export function ReviewStep({ locale, draft, evidence, extractions, onBack, onPrepare }: Props) {
  ...
  return <section className="page form-page">
    ...
    <section className={`route-preview ${route.confidence}`}><div className="eyebrow">{text.nextRoute} · {confidenceLabel(route.confidence, locale)}</div><h2>{route.routeName}</h2><p>{route.recommendedAction}</p><div className="route-details"><div><strong>{text.why}</strong><ul>{route.matchingFacts.map((fact) => <li key={fact}>{fact}</li>)}</ul></div><div><strong>{text.stillNeeded}</strong>{route.unmetPrerequisites.length ? <ul>{route.unmetPrerequisites.map((item) => <li key={item}>{item}</li>)}</ul> : <p>{text.nothingNeeded}</p>}</div></div><small><a href={route.sourceUrl} target="_blank" rel="noreferrer">{text.viewSource(route.sourceType === 'official')}</a> · {route.source} · {text.version} {route.ruleVersion} · {text.checked} {route.sourceChecked}. {text.acceptance}</small></section>
...
const reviewText = {
  en: { eyebrow: 'Buktiva Check', title: 'Review the record.', lede: 'This check uses the selected case type and included evidence. It identifies gaps; it does not decide whether the claim will succeed.', missing: (count: number) => `required ${count === 1 ? 'item' : 'items'} missing`, completeness: 'Completeness', completenessCopy: 'Each prompt names the product rule that caused it.', conflicts: 'Confirmed facts need review', required: 'required', useful: 'useful', source: 'Source', timeline: 'Buktiva Timeline', timelineCopy: 'Dates come only from confirmed case details or your evidence descriptions.', dateUnknown: 'Date unknown', confirmedDetail: 'confirmed case detail', userEvidence: 'user-described evidence', nextRoute: 'Next route', why: 'Why this result', stillNeeded: 'Still needed', nothingNeeded: 'Nothing for this initial route.', viewSource: (official: boolean) => `View ${official ? 'official source' : 'versioned product rule'}`, version: 'Version', checked: 'Checked', acceptance: 'The receiving body determines acceptance.', back: '← Evidence', resolveConflicts: 'Resolve fact conflicts', completeRequired: 'Complete required items', prepareRequest: 'Prepare merchant request', manualReview: 'Manual review needed' },
  ms: { eyebrow: 'Semakan Buktiva', title: 'Semak rekod.', lede: 'Semakan ini menggunakan jenis kes yang dipilih dan bukti yang disertakan. Ia mengenal pasti jurang; ia tidak menentukan sama ada tuntutan akan berjaya.', missing: (count: number) => `${count} perkara wajib belum lengkap`, completeness: 'Kelengkapan', completenessCopy: 'Setiap gesaan menamakan peraturan produk yang menyebabkannya.', conflicts: 'Fakta yang disahkan perlu disemak', required: 'wajib', useful: 'berguna', source: 'Sumber', timeline: 'Garis masa Buktiva', timelineCopy: 'Tarikh datang hanya daripada butiran kes yang disahkan atau keterangan bukti anda.', dateUnknown: 'Tarikh tidak diketahui', confirmedDetail: 'butiran kes disahkan', userEvidence: 'bukti yang diterangkan pengguna', nextRoute: 'Laluan seterusnya', why: 'Mengapa hasil ini', stillNeeded: 'Masih diperlukan', nothingNeeded: 'Tiada untuk laluan awal ini.', viewSource: (official: boolean) => `Lihat ${official ? 'sumber rasmi' : 'peraturan produk berversikan'}`, version: 'Versi', checked: 'Disemak', acceptance: 'Badan penerima menentukan penerimaan.', back: '← Bukti', resolveConflicts: 'Selesaikan konflik fakta', completeRequired: 'Lengkapkan perkara wajib', prepareRequest: 'Sediakan permintaan peniaga', manualReview: 'Semakan manual diperlukan' },
} as const
