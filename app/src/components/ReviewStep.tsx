...
import { buildTimeline, checkCompleteness, findFactConflicts, findTimelineWarnings, timelineDetail, timelineLabel, timelineSource } from '../domain/caseReview'
...
import { buildReviewBrief, downloadReviewBrief } from '../data/reviewBrief'

type Props = { locale: Locale; caseId: string; draft: CaseDraft; evidence: EvidenceMetadata[]; extractions: EvidenceExtraction[]; onBack: () => void; onPrepare: (route: RouteEvaluation) => void }

export function ReviewStep({ locale, caseId, draft, evidence, extractions, onBack, onPrepare }: Props) {
  ...
  const timeline = buildTimeline(draft, evidence, extractions)
  ...
  function exportBrief() {
    setExportingBrief(true); setBriefError('')
    try {
      downloadReviewBrief(buildReviewBrief({ case: draft, locale, checks, timeline, conflicts, route, evidence, extractions }), caseId)
    } catch {
      setBriefError(text.reviewBriefError)
    } finally {
  ...
  return <section className="page form-page">
    ...
    <div className="actions split"><button className="secondary" onClick={onBack}>{text.back}</button><div className="button-group"><button className="secondary" onClick={exportBrief} disabled={exportingBrief}>{exportingBrief ? text.exportingBrief : text.exportBrief}</button><button className="primary" onClick={() => onPrepare(route)} disabled={route.confidence !== 'supported' || missingRequired > 0 || conflicts.length > 0}>{conflicts.length > 0 ? text.resolveConflicts : missingRequired > 0 ? text.completeRequired : route.confidence === 'supported' ? text.prepareRequest : text.manualReview}</button></div></div>
  </section>
}

const reviewText = {
  en: { eyebrow: 'Aduen Check', title: 'Review the record.', lede: 'This check uses the selected case type and included evidence. It identifies gaps; it does not decide whether the claim will succeed.', missing: (count: number) => `required ${count === 1 ? 'item' : 'items'} missing`, completeness: 'Completeness', completenessCopy: 'Each prompt names the product rule that caused it.', conflicts: 'Confirmed facts need review', required: 'required', useful: 'useful', source: 'Source', timeline: 'Aduen Timeline', timelineCopy: 'Dates come from confirmed case details, your evidence descriptions, or dates you confirmed from extracted evidence.', dateUnknown: 'Date unknown', confirmedDetail: 'confirmed case detail', userEvidence: 'user-described evidence', nextRoute: 'Next route', why: 'Why this result', stillNeeded: 'Still needed', nothingNeeded: 'Nothing for this initial route.', possibleDestinations: 'Possible official destinations', destinationNote: 'These links are starting points only. Check the current requirements and eligibility yourself.', viewSource: (official: boolean) => `View ${official ? 'official source' : 'versioned product rule'}`, version: 'Version', checked: 'Checked', acceptance: 'The receiving body determines acceptance.', back: '← Evidence', resolveConflicts: 'Resolve fact conflicts', completeRequired: 'Complete required items', prepareRequest: 'Prepare merchant request', manualReview: 'Manual review needed', exportBrief: 'Export manual review brief', exportingBrief: 'Preparing review brief...', reviewBriefError: 'The manual review brief could not be exported.' },
  ms: { eyebrow: 'Semakan Aduen', title: 'Semak rekod.', lede: 'Semakan ini menggunakan jenis kes yang dipilih dan bukti yang disertakan. Ia mengenal pasti jurang; ia tidak menentukan sama ada tuntutan akan berjaya.', missing: (count: number) => `${count} perkara wajib belum lengkap`, completeness: 'Kelengkapan', completenessCopy: 'Setiap gesaan menamakan peraturan produk yang menyebabkannya.', conflicts: 'Fakta yang disahkan perlu disemak', required: 'wajib', useful: 'berguna', source: 'Sumber', timeline: 'Garis masa Aduen', timelineCopy: 'Tarikh datang daripada butiran kes yang disahkan, keterangan bukti anda, atau tarikh yang anda sahkan daripada ekstrak bukti.', dateUnknown: 'Tarikh tidak diketahui', confirmedDetail: 'butiran kes disahkan', userEvidence: 'bukti yang diterangkan pengguna', nextRoute: 'Laluan seterusnya', why: 'Mengapa hasil ini', stillNeeded: 'Masih diperlukan', nothingNeeded: 'Tiada untuk laluan awal ini.', possibleDestinations: 'Destinasi rasmi yang mungkin', destinationNote: 'Pautan ini hanyalah titik permulaan. Semak sendiri keperluan dan kelayakan semasa.', viewSource: (official: boolean) => `Lihat ${official ? 'sumber rasmi' : 'peraturan produk berversikan'}`, version: 'Versi', checked: 'Disemak', acceptance: 'Badan penerima menentukan penerimaan.', back: '← Bukti', resolveConflicts: 'Selesaikan konflik fakta', completeRequired: 'Lengkapkan perkara wajib', prepareRequest: 'Sediakan permintaan peniaga', manualReview: 'Semakan manual diperlukan', exportBrief: 'Eksport ringkasan semakan manual', exportingBrief: 'Menyediakan ringkasan semakan...', reviewBriefError: 'Ringkasan semakan manual tidak dapat dieksport.' },
} as const
