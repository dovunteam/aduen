...
describe('local audit log', () => {
  it('records ordered sensitive-action events and can clear them', () => {
    ...
    const fourth = recordAuditEvent('hosted_case_imported', 'remote-case', 'structured hosted case copied into a new local draft', new Date('2026-09-21T00:03:00Z'))
    expect(listAuditEvents()).toEqual([first, second, third, fourth])
    clearAuditEvents()
  ...
  it('ignores malformed stored records', () => {
    localStorage.setItem('Aduen.audit-log.v1', JSON.stringify([{ action: 'unknown' }, { id: 'invalid-time', at: 'not-a-date', action: 'case_exported', targetId: 'case', detail: 'archive' }, { id: 'oversized', at: '2026-09-21T00:00:00.000Z', action: 'case_exported', targetId: 'case', detail: 'x'.repeat(241) }, { id: 'control', at: '2026-09-21T00:00:00.000Z', action: 'case_exported', targetId: 'case', detail: 'archive\nwith-control' }, { id: 'valid', at: '2026-09-21T00:00:00.000Z', action: 'case_exported', targetId: 'case', detail: 'archive' }]))
