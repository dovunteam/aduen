    ...
    <main>
      ...
      {step === 'case' && <section className="page form-page">
        ...
        <form onSubmit={saveCase}>
          <div className="form-section"><SectionTitle number="01" title="Transaction" copy="Identify the consumer, purchase, and seller." /><div className="fields two-col"><label>Your name or chosen case name<input required value={draft.consumerName} onChange={(e) => updateDraft('consumerName', e.target.value)} placeholder="Name used in the complaint" /></label><label>Your location<select required value={draft.consumerLocation} onChange={(e) => updateDraft('consumerLocation', e.target.value as CaseDraft['consumerLocation'])}><option value="">Select a location</option><option value="malaysia">Malaysia</option><option value="outside">Outside Malaysia</option></select></label><label>Seller or merchant<input required value={draft.seller} onChange={(e) => updateDraft('seller', e.target.value)} placeholder="Business or seller name" /></label><label>Seller location<select required value={draft.sellerLocation} onChange={(e) => updateDraft('sellerLocation', e.target.value as CaseDraft['sellerLocation'])}><option value="">Select if known</option><option value="malaysia">Malaysia</option><option value="outside">Outside Malaysia</option><option value="unknown">Unknown</option></select></label><label>Platform <span className="optional">Optional</span><input value={draft.platform} onChange={(e) => updateDraft('platform', e.target.value)} placeholder="Website, marketplace, or app" /></label><label>Purchase date<input required type="date" value={draft.purchaseDate} onChange={(e) => updateDraft('purchaseDate', e.target.value)} /></label><label>Amount paid (MYR)<input required min="0" step="0.01" type="number" value={draft.amount} onChange={(e) => updateDraft('amount', e.target.value)} placeholder="0.00" /></label><label>Payment method<select required value={draft.paymentMethod} onChange={(e) => updateDraft('paymentMethod', e.target.value)}><option value="">Select a method</option><option>Card</option><option>Bank transfer</option><option>E-wallet</option><option>Cash on delivery</option><option>Other</option></select></label><label>Order or reference number <span className="optional">Optional</span><input value={draft.orderReference} onChange={(e) => updateDraft('orderReference', e.target.value)} placeholder="Order ID or receipt number" /></label></div></div>
          <div className="form-section"><SectionTitle number="02" title="Purpose and problem" copy="This affects which routes may apply." /><div className="fields two-col"><label>Purchase purpose<select required value={draft.purpose} onChange={(e) => updateDraft('purpose', e.target.value as CaseDraft['purpose'])}><option value="">Select a purpose</option><option value="personal">Personal, household, or domestic</option><option value="business">Business or professional</option></select></label><label>Purchase category<select required value={draft.category} onChange={(e) => updateDraft('category', e.target.value as CaseDraft['category'])}><option value="">Select a category</option><option value="general_goods">General goods</option><option value="general_services">General services</option><option value="aviation">Airline or airport</option><option value="financial_service">Financial service</option><option value="healthcare">Healthcare</option><option value="professional_service">Regulated professional service</option><option value="land">Land or property</option><option value="other">Other or uncertain</option></select></label><label>What went wrong?<select required value={draft.issue} onChange={(e) => updateDraft('issue', e.target.value as CaseDraft['issue'])}><option value="">Select the main issue</option><option value="non_delivery">Goods or services not received</option><option value="mismatch">Materially different from advertised</option><option value="missing_refund">Promised refund not received</option><option value="cancellation">Cancellation or billing problem</option><option value="uncertain">I am not sure</option></select></label></div></div>
          ...
          <div className="form-section"><SectionTitle number="04" title="Promise and contact" copy="Record what was due and whether the merchant has heard from you." /><div className="fields two-col"><label>Promised date <span className="optional">If known</span><input type="date" value={draft.promisedDate} onChange={(e) => updateDraft('promisedDate', e.target.value)} /></label><label>Merchant contact<select required value={draft.contactHistory} onChange={(e) => updateDraft('contactHistory', e.target.value as CaseDraft['contactHistory'])}><option value="">Select contact status</option><option value="none">I have not sent a clear written request</option><option value="contacted">I contacted them; no response yet</option><option value="responded">The merchant responded</option></select></label>{draft.contactHistory && draft.contactHistory !== 'none' && <label>Date of latest contact<input required type="date" value={draft.contactDate} onChange={(e) => updateDraft('contactDate', e.target.value)} /></label>}</div></div>
