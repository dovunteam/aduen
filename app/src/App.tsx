  ...
  return <div className={`app-shell ${step === 'welcome' || step === 'workspace' ? 'is-welcome' : 'is-workflow'}`}>
    ...
    <main>
      {step !== 'data' && step !== 'workspace' && step !== 'welcome' && <nav className="progress" aria-label={text.progressLabel}>
        ...
        {text.progress.map((label, index) => <div aria-current={index + 1 === progress ? 'step' : undefined} className={index + 1 <= progress ? 'progress-item active' : 'progress-item'} key={label}><span>{String(index + 1).padStart(2, '0')}</span>{label}</div>)}
      </nav>}
