...
describe('initial routing', () => {
  it('recommends merchant-first when no contact is recorded', () => {
    const route = evaluateInitialRoute({ ...EMPTY_DRAFT, purpose: 'personal', issue: 'non_delivery', remedy: 'refund', contactHistory: 'none' }, [])
  ...
  it('does not guess an escalation after merchant contact', () => {
    const route = evaluateInitialRoute({ ...EMPTY_DRAFT, purpose: 'personal', contactHistory: 'contacted' }, [])
