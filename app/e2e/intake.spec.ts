...
test('a supported draft preserves original evidence and resumes at the evidence stage', async ({ page }) => {
  ...
  await fillCase(page)
  await page.getByRole('button', { name: /Add evidence/ }).click()
  await page.getByLabel('Original file').setInputFiles({ name: 'synthetic-receipt.txt', mimeType: 'text/plain', buffer: Buffer.from('Synthetic order receipt. Total MYR 125.50. <script>window.evidenceExecuted=true</script>') })
  await page.getByLabel('Event date').fill('2026-08-01')
