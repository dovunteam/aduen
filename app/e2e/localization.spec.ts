...
test('Bahasa Malaysia case details preserve stable domain values', async ({ page }) => {
  ...
  await addBmEvidence(page, 'message', 'mesej.txt', 'No. rujukan: ADU-1234. Tarikh pembelian: 3 September 2026. Dibayar RM120.00 pada 3 September 2026. Nama pelanggan: Pengguna Contoh. Saya memohon bayaran balik.')
  await page.getByRole('button', { name: 'Semak kes' }).click()
  await expect(page.getByRole('heading', { name: 'Semak setiap calon.' })).toBeVisible()
  await expect(page.getByText('plain-text-v3', { exact: true })).toBeVisible()
  ...
  await expect(page.getByText('plain-text-v3').last()).toBeVisible()
