...
The browser prototype stores structured case data in `localStorage`; evidence originals and metadata use separate IndexedDB stores. Evidence, extracted text, and complaint packs stay in the browser.

When `VITE_API_BASE_URL` and the optional `VITE_OIDC_*` settings are configured, Data Controls exposes OIDC sign-in and a manual hosted-copy action. Each save requires a fresh acknowledgement and sends only the structured case record. Later saves replace that account's copy using its current API revision. Configure only a reviewed identity-provider client, API audience, recovery, and redirect policies.

Users can list or delete hosted copies, or copy a structured hosted case into a new local draft when this browser has no case-related data. Copying creates a new local case ID and leaves the hosted copy unchanged. Evidence, extracted text, complaint packs, consent, and submission details are not migrated, so this does not provide full cross-device case use. Local deletion does not delete hosted copies.

The separate synthetic-data case API is documented in [`service/api`](../service/api/README.md). Do not use the prototype or API foundation for real consumer records. Static hosts that support the `_headers` convention can apply the baseline browser security headers in `public/_headers`. Production builds rewrite `connect-src` for the configured API and OIDC origins; add any separate discovery or token endpoint origins to `VITE_CSP_CONNECT_ORIGINS`. The builder validates these origins, but deployed response headers still need verification.
