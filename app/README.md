...
The browser prototype stores structured case data in `localStorage`; evidence originals and metadata use separate IndexedDB stores. When both `VITE_API_BASE_URL` and the optional `VITE_OIDC_*` settings are configured, Data Controls exposes generic OIDC sign-in and a manual hosted-copy action. Every save requires a fresh, unchecked acknowledgement. It sends only the structured case record, and a later save replaces that account's copy using its current API revision. Evidence, extracted text, and complaint packs remain local. Local deletion does not delete the hosted copy; hosted account deletion and retrieval from another device are not implemented. Configure only a reviewed provider client, API audience, recovery, and redirect policies. The separate synthetic-data case API is documented in [`service/api`](../service/api/README.md). Do not use the prototype or API foundation for real consumer records. Static hosting platforms that support the `_headers` convention can apply the baseline browser security headers in `public/_headers`; verify the deployed response headers because the build cannot enforce host configuration.
