...
3. Build the API image and review the dependency audit and CI scan for high or critical image vulnerabilities; scan the final image according to the selected runtime’s security policy and record its immutable digest. Build the browser application with the production API base URL and OIDC public settings. Register both browser redirect URIs with the issuer.
