...
3. Build the API image and review the dependency audit and CI scan for high or critical image vulnerabilities. Retain the CI artifact containing the tested image's CycloneDX SBOM and image ID, scan the final image according to the selected runtime's security policy, and record its immutable registry digest. Build the browser application with the production API base URL and OIDC public settings. Register both browser redirect URIs with the issuer.
...
`service/api/ops/aduen-alerts.yml` provides portable Prometheus alert rules for a scrape outage, sustained 5xx responses, elevated request latency, and PostgreSQL pool waiters. Configure the scraper job label as `aduen-api`, restrict scraper network access to `/metrics`, and set the bearer token through the Prometheus secret facility. Route alert notifications through the selected operations system; the rules do not create notification channels or alerts for scheduled job outcomes.
