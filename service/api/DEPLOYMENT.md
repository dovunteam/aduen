...
Each API process opens a PostgreSQL pool of up to `DATABASE_POOL_MAX` connections (default `10`, allowed range `1` to `100`). Set it and the maximum replica count so their product stays within the connection budget allocated to API replicas, leaving capacity for migrations, maintenance, backups, monitoring, and database administration. Production rate-limit counters are shared in PostgreSQL: requests are limited to 120 per 60-second window per trusted client IP before authentication, and to 120 per authenticated subject after authentication. Both keys are HMAC-SHA256 digests; the subject itself is not stored in the limiter table. All replicas must use the same `RATE_LIMIT_HMAC_KEY`. For rotation, set the new active key in `RATE_LIMIT_HMAC_KEY` and the old key in `RATE_LIMIT_HMAC_KEY_PREVIOUS` on every replica before rolling them; both namespaces are incremented and the higher count enforces each limit. After every replica has used both keys for at least one 60-second window, remove `RATE_LIMIT_HMAC_KEY_PREVIOUS`.
...
| `DATABASE_POOL_MAX` | Maximum PostgreSQL connections per API replica, whole number from 1 to 100; default is 10 |
| `DATABASE_SSL` | `true` |
